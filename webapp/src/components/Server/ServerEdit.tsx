...
export const ServerEdit: React.FC<ServerEditProps> = () => {
  ...
  const handleEditServer = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const updatedData = {
      ...
      PersistentKeepalive: persistentKeepalive
        ? parseInt(persistentKeepalive)
        : serverData.PersistentKeepalive,
    };
    ...
    try {
      ...
      setConfirmationMessage(
        "Error updating server configuration. Please try again."
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div>
      <label htmlFor="server-edit" className="">
        Edit
      </label>
      ...
      <div className="modal">
        <div className="modal-box relative w-full bg-gray-900 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-80 border border-gray-100">
          ...
          <div>
            <form onSubmit={(e) => handleEditServer(e)}>
              ...
              {confirmationMessage && (
                <p className="mt-3 text-blue-200">{confirmationMessage}</p>
              )}
            </form>
          </div>
        </div>
